...
contract UserID is IERC7231,ERC721 {
    using { toSlice } for string;
    mapping (uint256 => bytes32) internal _idMultiIdentitiesRootBinding;
    mapping(uint256 => mapping(bytes32 => bool)) internal _idSignatureSetting;

    constructor(string memory _name, string memory _symbol) ERC721(_name, _symbol) {}

    // @dev requires the tokenId to be not minted.
    function mint() external {
        uint256 tokenId = uint256(keccak256(abi.encodePacked(_msgSender())));
     ...
     * @param id nft id
     * @param multiIdentitiesRoot msg hash to verify
